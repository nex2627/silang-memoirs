...
alter type user_role add value if not exists 'eic';
alter type user_role add value if not exists 'aeic';
alter type user_role add value if not exists 'art_director';
alter type user_role add value if not exists 'finance_manager';
alter type user_role add value if not exists 'assistant_finance_manager';
...
alter table public.schedules add column if not exists posted_at timestamptz;
...
alter table public.schedule_members add column if not exists sorted boolean not null default false;
...
begin
 if new.department='Media' and new.status='completed' and (old.status is distinct from new.status) then
   ...
   select id,'Media event sorted — ready for captions & color grading',new.title,'colorist' from public.profiles where role in ('head_colorist','staff_colorist','head_writer');
 end if;
...
-- Colorist finishing a grade notifies Head Layout that the event is ready for posting-cover layout.
create or replace function public.notify_layout_ready() returns trigger language plpgsql security definer set search_path=public as $$
declare ev_title text;
begin
 if new.department='Colorist' and new.status='completed' and (old.status is distinct from new.status) and new.schedule_id is not null then
   select title into ev_title from public.schedules where id=new.schedule_id;
   insert into public.notifications(user_id,title,body,kind)
   select id,'Color grade done — ready for layout',coalesce(ev_title,new.title),'layout' from public.profiles where role='head_layout';
 end if;
 return new;
end; $$;
drop trigger if exists on_task_colorist_done on public.tasks;
create trigger on_task_colorist_done after update on public.tasks for each row execute procedure public.notify_layout_ready();
...
-- Per-photographer sorting: once every assigned member on a Media event has marked their own portion sorted, the event itself flips to 'completed' (which fires the existing writer/colorist handoff notifications).
create or replace function public.check_all_sorted() returns trigger language plpgsql security definer set search_path=public as $$
declare remaining int; ev_department text;
begin
  if new.sorted is distinct from old.sorted and new.sorted=true then
    select department into ev_department from public.schedules where id=new.schedule_id;
    if ev_department='Media' then
      select count(*) into remaining from public.schedule_members where schedule_id=new.schedule_id and sorted=false;
      if remaining=0 then
        update public.schedules set status='completed' where id=new.schedule_id and status<>'completed';
      end if;
    end if;
  end if;
  return new;
end; $$;
drop trigger if exists on_member_sorted on public.schedule_members;
create trigger on_member_sorted after update on public.schedule_members for each row execute procedure public.check_all_sorted();
...
create or replace function public.is_admin_like() returns boolean language sql stable security definer set search_path=public as $$ select public.get_my_role() in ('admin','eic','aeic','art_director') $$;
create or replace function public.is_finance() returns boolean language sql stable security definer set search_path=public as $$ select public.get_my_role() in ('finance_manager','assistant_finance_manager') $$;

-- Profiles (edit / password-reset rights stay admin-only, per spec — EIC/AEIC/Art Director do not get Settings)
...
-- Tasks
create policy tasks_select on public.tasks for select to authenticated using (public.is_admin_like() or public.is_finance() or (public.get_my_role() like 'head_%' and department=public.get_my_department()) or assigned_to=auth.uid() or (public.get_my_role()='head_layout' and department='Colorist'));
create policy tasks_insert on public.tasks for insert to authenticated with check (public.is_admin_like() or (public.get_my_role() like 'head_%' and department=public.get_my_department()));
create policy tasks_update on public.tasks for update to authenticated using (public.is_admin_like() or (public.get_my_role() like 'head_%' and department=public.get_my_department()) or assigned_to=auth.uid()) with check (public.is_admin_like() or (public.get_my_role() like 'head_%' and department=public.get_my_department()) or assigned_to=auth.uid());
create policy tasks_delete on public.tasks for delete to authenticated using (public.is_admin_like() or (public.get_my_role() like 'head_%' and department=public.get_my_department()));

-- Schedules
create policy schedules_select on public.schedules for select to authenticated using (
  public.is_admin_like() or public.is_finance()
...
create policy schedules_insert on public.schedules for insert to authenticated with check (public.is_admin_like() or (public.get_my_role() like 'head_%' and department=public.get_my_department()));
create policy schedules_update on public.schedules for update to authenticated using (
  public.is_admin_like() or (public.get_my_role() like 'head_%' and department=public.get_my_department())
) with check (
  public.is_admin_like() or (public.get_my_role() like 'head_%' and department=public.get_my_department())
);
create policy schedules_delete on public.schedules for delete to authenticated using (public.is_admin_like() or (public.get_my_role() like 'head_%' and department=public.get_my_department()));

-- Schedule members
create policy schedule_members_select on public.schedule_members for select to authenticated using (public.is_admin_like() or public.is_finance() or public.schedule_department(schedule_id)=public.get_my_department() or member_id=auth.uid());
create policy schedule_members_insert on public.schedule_members for insert to authenticated with check (public.is_admin_like() or public.schedule_department(schedule_id)=public.get_my_department());
create policy schedule_members_update on public.schedule_members for update to authenticated using (public.is_admin_like() or public.schedule_department(schedule_id)=public.get_my_department() or member_id=auth.uid()) with check (public.is_admin_like() or public.schedule_department(schedule_id)=public.get_my_department() or member_id=auth.uid());
create policy schedule_members_delete on public.schedule_members for delete to authenticated using (public.is_admin_like() or public.schedule_department(schedule_id)=public.get_my_department());

-- Outputs (task-mode departments only, enforced in UI — table itself is open per-department)
create policy outputs_select on public.outputs for select to authenticated using (public.is_admin_like() or public.is_finance() or (public.get_my_role() like 'head_%' and department=public.get_my_department()) or user_id=auth.uid());
...
create policy outputs_update on public.outputs for update to authenticated using (public.is_admin_like() or user_id=auth.uid()) with check (public.is_admin_like() or user_id=auth.uid());
create policy outputs_delete on public.outputs for delete to authenticated using (public.is_admin_like() or user_id=auth.uid());

-- Resources
create policy resources_select on public.resources for select to authenticated using (public.is_admin_like() or public.is_finance() or department is null or department=public.get_my_department());
create policy resources_insert on public.resources for insert to authenticated with check (public.is_admin_like() or (public.get_my_role() like 'head_%' and (department=public.get_my_department() or department is null)));
create policy resources_update on public.resources for update to authenticated using (public.is_admin_like() or (public.get_my_role() like 'head_%' and department=public.get_my_department())) with check (public.is_admin_like() or (public.get_my_role() like 'head_%' and department=public.get_my_department()));
create policy resources_delete on public.resources for delete to authenticated using (public.is_admin_like() or (public.get_my_role() like 'head_%' and department=public.get_my_department()));

-- Integrations (Google Sheet link per scope — heads/admin-like set it, everyone with backup access can read it)
create policy integrations_select on public.integrations for select to authenticated using (public.is_admin_like() or public.is_finance() or (public.get_my_role() like 'head_%' and scope=public.get_my_department()));
create policy integrations_insert on public.integrations for insert to authenticated with check (public.is_admin_like() or (public.get_my_role() like 'head_%' and scope=public.get_my_department()));
create policy integrations_update on public.integrations for update to authenticated using (public.is_admin_like() or (public.get_my_role() like 'head_%' and scope=public.get_my_department())) with check (public.is_admin_like() or (public.get_my_role() like 'head_%' and scope=public.get_my_department()));
...
create policy notifications_insert on public.notifications for insert to authenticated with check (public.is_admin_like());
...
create policy announcements_insert on public.announcements for insert to authenticated with check (public.is_admin_like());
