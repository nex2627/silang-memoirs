...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const adminLike=['admin','eic','aeic','art_director']
  if(!adminLike.includes(profile?.role))return new Response(JSON.stringify({error:'Not permitted to create accounts'}),{status:403,headers:{...cors,'Content-Type':'application/json'}})
  const body=await req.json(); const allowed=['admin','eic','aeic','art_director','finance_manager','assistant_finance_manager','head_media','head_layout','head_writer','head_researcher','head_colorist','staff_media','staff_layout','staff_writer','staff_researcher','staff_colorist']
