...
import {
  ...
  Download, Wand2, KeyRound, FolderKanban, Megaphone, Trash2, BellOff, Pencil, History
} from 'lucide-react'
...
const TIME_OPTIONS=(()=>{ const out=[]; for(let i=0;i<96;i++){ const mins=(6*60+i*15)%1440; const h=Math.floor(mins/60); const m=mins%60; const hh=String(h).padStart(2,'0'); const mm=String(m).padStart(2,'0'); const label=new Date(2000,0,1,h,m).toLocaleTimeString([], {hour:'numeric',minute:'2-digit'}); out.push([`${hh}:${mm}`,label]) } return out })()
...
function LoadingScreen(){
  return <div className="center-screen loader-screen">
    <div className="loader-wrap">
      <div className="loader-orbit"><img className="loader-logo" src={LOGO} alt=""/><span className="loader-dot"/></div>
      <div className="loader-text">SILANG MEMOIRS</div>
    ...
    <main className={`main ${collapsed?'sidebar-collapsed':''}`}>
      <header className="topbar">
        <div className="mobile-account">
          <div className="avatar">{profile.avatar_url?<img src={profile.avatar_url} alt="" style={{width:'100%',height:'100%',borderRadius:'50%',objectFit:'cover'}}/>:initials(profile.full_name)}</div>
          <div><strong>{profile.full_name}</strong><span>{ROLE_META[role]?.label || role}</span></div>
...
function AdminDashboard({profile}){
  ...
  return <div>
    ...
    <PostingTracker profile={profile}/>
  </div>
}

/* ---------------- Posting Tracker: cross-department status for every Media event ---------------- */
function PostingTracker({profile}){
  const [rows,setRows]=useState([]); const [loading,setLoading]=useState(true)
  const canPost=isAdminLike(profile.role)||profile.role==='head_layout'
  useEffect(()=>{ load() },[])
  async function load(){
    setLoading(true)
    ...
    const statusFor=(eventId,dept)=>{
      const ts=(tasks||[]).filter(x=>x.schedule_id===eventId && x.department===dept)
      if(!ts.length) return 'not started'
    ...
    setRows((events||[]).map(e=>({
      ...
      caption:statusFor(e.id,'Writer'), colorgrade:statusFor(e.id,'Colorist'), layout:statusFor(e.id,'Layout Artist'),
      posted:!!e.posted_at, postedOn:e.posted_at?new Date(e.posted_at).toLocaleDateString():'—'
    })))
    setLoading(false)
  }
  async function markPosted(id){ await supabase.from('schedules').update({posted_at:new Date().toISOString()}).eq('id',id); load() }
  return <div className="panel table-panel" style={{marginTop:14}}>
    ...
    <div style={{overflowX:'auto'}}>
      ...
      {loading ? <div className="empty">Loading…</div> : rows.map(r=><div className="table-row posting-row" key={r.id}>
        ...
        <span className={`status ${r.caption==='completed'?'completed':r.caption==='in progress'?'in_progress':'scheduled'}`}>{r.caption}</span>
        <span className={`status ${r.colorgrade==='completed'?'completed':r.colorgrade==='in progress'?'in_progress':'scheduled'}`}>{r.colorgrade}</span>
        <span className={`status ${r.layout==='completed'?'completed':r.layout==='in progress'?'in_progress':'scheduled'}`}>{r.layout}</span>
        <span>{r.posted ? <span className="status completed">posted</span> : (canPost && r.layout==='completed' ? <button className="small-btn" onClick={()=>markPosted(r.id)}>Mark posted</button> : <span className="status scheduled">not posted</span>)}</span>
...
function HoursDashboard({profile}){
  ...
  useEffect(()=>{ load() },[profile.id])
  async function load(){
    let q=supabase.from('schedules').select('*,schedule_members(member_id,sorted,profiles(full_name))').eq('department',profile.department).order('start_at',{ascending:true})
    ...
    if(staff){
      const unsorted=rows.filter(s=>{ const my=(s.schedule_members||[]).find(m=>m.member_id===profile.id); return my && !my.sorted && new Date(s.start_at)<=now })
      setToSort(unsorted)
    }
  }
  async function markSorted(scheduleId){ await supabase.from('schedule_members').update({sorted:true}).eq('schedule_id',scheduleId).eq('member_id',profile.id); load() }
  const next=items[0]
  return <div>
    <WelcomeHero profile={profile}/>
    ...
    {staff && toSort.length>0 && <div className="panel task-panel" style={{marginBottom:14}}>
      <div className="panel-title"><h2>To sort</h2></div>
      <div className="task-list">{toSort.map(s=><div className="task-row" key={s.id}><div className="task-icon media"><CalendarDays size={16}/></div>
        <div className="task-info"><strong>{s.title}</strong><span>{new Date(s.start_at).toLocaleDateString()}</span></div>
        <button className="small-btn" onClick={()=>markSorted(s.id)}>Sorted</button></div>)}</div>
    </div>}
    {staff && next && <div className="panel task-panel" style={{marginBottom:14}}>
      ...
      <div className="member-chips" style={{justifyContent:'flex-start'}}>{(next.schedule_members||[]).map(m=><span key={m.member_id}>{m.profiles?.full_name}{m.sorted?' ✓':''}</span>)}</div>
    </div>}
    <div className="panel task-panel">
      ...
      <div className="task-list">{items.map(s=><div className="task-row" key={s.id}><div className="task-icon media"><CalendarDays size={16}/></div>
        ...
        <span className={`status ${s.status}`}>{s.status==='completed'?'sorted':s.status}</span></div>)}
        {!items.length && <div className="empty">Nothing scheduled yet.</div>}</div>
    </div>
    {!staff && <PostingTracker profile={profile}/>}
  </div>
}
  ...
  return <div>
    ...
    {stageCfg && <div className="panel" style={{marginBottom:14}}>
      ...
      {avail.map(e=>{ const picked=[...(picks[e.id]||[])]; return <div style={{padding:'16px 21px',borderTop:'1px solid var(--border)'}} key={e.id}>
        <div style={{marginBottom:10}}><strong style={{display:'block',fontSize:13}}>{e.title}</strong><span style={{fontSize:11,color:'var(--text-dim)'}}>{new Date(e.start_at).toLocaleDateString()}</span></div>
        <div className="assign-row">
          <button className="small-btn" onClick={()=>assignStage(e.id,e.title,[profile.id])}>Assign to myself</button>
          <div className="token-row" style={{flex:1}}>{teammates.map(m=><button type="button" key={m.id} className={`token ${picked.includes(m.id)?'active':''}`} onClick={()=>setPickList(e.id, picked.includes(m.id)?picked.filter(x=>x!==m.id):[...picked,m.id])}>{m.full_name}</button>)}
            {!teammates.length && <span style={{fontSize:11,color:'var(--text-dim)'}}>No staff in this department yet.</span>}</div>
          {picked.length>0 && <button className="small-btn" onClick={()=>assignStage(e.id,e.title,picked)}>Assign selected ({picked.length})</button>}
        </div>
      </div> })}
    </div>}
    <div className="panel task-panel">
      <div className="panel-title"><h2>{staff?'My task':'Recent tasks'}</h2></div>
      <div className="task-list">{(staff?undone:rows).map(t=>{ const mine=t.status!=='completed' && t.assigned_to===profile.id; return <div className="task-row" key={t.id}><div className="task-icon layout"><CheckSquare size={16}/></div>
        ...
        {(staff||mine) ? <button className="small-btn" onClick={()=>markDone(t.id)}>{t.department==='Colorist'?'Colorgraded':'Done'}</button> : <span className={`status ${t.status}`}>{t.status.replace('_',' ')}</span>}</div> })}
...
function EditTaskModal({task,onClose,onSaved}){
  const [form,setForm]=useState({title:task.title,description:task.description||'',assignedIds:task.assigned_to?[task.assigned_to]:[],due_date:task.due_date||'',assigned_date:task.assigned_date||''})
  const [members,setMembers]=useState([])
  const [busy,setBusy]=useState(false)
  useEffect(()=>{ (async()=>{ const {data}=await supabase.from('profiles').select('id,full_name,department,role').eq('department',task.department); setMembers(data||[]) })() },[])
  async function save(e){ e.preventDefault(); setBusy(true)
    const {error}=await supabase.from('tasks').update({ title:form.title, description:form.description||null, assigned_to:form.assignedIds[0]||null, due_date:form.due_date||null, assigned_date:form.assigned_date||null }).eq('id',task.id)
    setBusy(false); if(error) notifyDialog(error.message); else { onClose(); onSaved() }
  }
  const groups=[{label:'',people:members.filter(m=>m.role===DEPT_STAFF_ROLE[task.department])},{label:'HEAD',people:members.filter(m=>m.role===DEPT_HEAD_ROLE[task.department])}]
  return <Modal title="Edit task" onClose={onClose}><form onSubmit={save}>
    <label>Title<input required value={form.title} onChange={e=>setForm({...form,title:e.target.value})}/></label>
    <label>Description<textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/></label>
    <label>Assign to</label>
    <PersonPicker groups={groups} value={form.assignedIds} multi={false} onChange={ids=>setForm({...form,assignedIds:ids})}/>
...
function CalendarPage({profile}){
  ...
  async function load(){
    ...
    if(effMode==='hours' || effMode==='all'){
      let sq=supabase.from('schedules').select('*,schedule_members(member_id,sorted,profiles(full_name))').gte('start_at',rangeStart.toISOString()).lt('start_at',rangeEnd.toISOString()).order('start_at')
  ...
  return <div>
    ...
    {selected && <Modal title={selected.date.toLocaleDateString([], {weekday:'long',month:'long',day:'numeric'})} onClose={()=>setSelected(null)}>
      {selected.items.map(e=><div key={e.id} className="day-detail-row">
        ...
        {e.type==='schedule' && e.raw.schedule_members?.length>0 && <div className="member-chips">{e.raw.schedule_members.map(m=><span key={m.member_id}>{m.profiles?.full_name}{m.sorted?' ✓ sorted':' • pending'}</span>)}</div>}
        <div style={{marginTop:8,display:'flex',gap:8,flexWrap:'wrap'}}>
          ...
          {can && <button className="ghost-btn" onClick={()=>setEditItem(e)}><Pencil size={13}/> Edit</button>}
    ...
    {editItem?.type==='schedule' && <EditScheduleModal schedule={editItem.raw} onClose={()=>setEditItem(null)} onSaved={()=>{ load(); setSelected(null) }}/>}
    {editItem?.type==='task' && <EditTaskModal task={editItem.raw} onClose={()=>setEditItem(null)} onSaved={()=>{ load(); setSelected(null) }}/>}
  </div>
}
...
function ScheduleModal({profile,onClose,onSaved}){
  ...
  return <Modal title="New event" onClose={onClose}><form onSubmit={save}>
    ...
    <div className="form-row">
      <label>Start time<TimeSelect required value={form.start} onChange={v=>setForm({...form,start:v})}/></label>
      <label>End time<TimeSelect required value={form.end} onChange={v=>setForm({...form,end:v})}/></label>
    </div>
...
function Tracker({profile}){
  ...
  useEffect(()=>{ load() },[profile.id])
  async function load(){
    let q=supabase.from('schedules').select('*,schedule_members(member_id,sorted,profiles(full_name))').eq('department',profile.department).in('status',['completed','done']).order('start_at',{ascending:false})
...
function AnnouncementModal({profile,onClose,onSaved}){
  ...
  const [people,setPeople]=useState([]); const [busy,setBusy]=useState(false)
  useEffect(()=>{ (async()=>{ const {data}=await supabase.from('profiles').select('id,full_name,role,department').order('full_name'); setPeople(data||[]) })() },[])
  async function save(e){ e.preventDefault(); setBusy(true)
    let recipients=[]
  ...
  return <Modal title="New announcement" onClose={onClose}><form onSubmit={save}>
    ...
    {form.audience==='custom' && <><label>Recipients</label>
      <PersonPicker groups={[
        {label:'MEDIA',people:people.filter(p=>p.department==='Media')},
        {label:'COLORIST',people:people.filter(p=>p.department==='Colorist')},
        {label:'LAYOUT ARTIST',people:people.filter(p=>p.department==='Layout Artist')},
        {label:'WRITER',people:people.filter(p=>p.department==='Writer')},
        {label:'RESEARCHER',people:people.filter(p=>p.department==='Researcher')},
        {label:'EDITORIAL',people:people.filter(p=>['admin','eic','aeic','art_director','finance_manager','assistant_finance_manager'].includes(p.role))},
      ]} value={form.target} multi={true} onChange={ids=>setForm({...form,target:ids})}/>
    </>}
